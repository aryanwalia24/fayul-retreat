...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedSelectedRooms) {
      const selectedRooms = JSON.parse(storedSelectedRooms);
      const totalRoomCharge = selectedRooms.reduce((acc, room) => {
        const price = parseFloat(room.split('₹')[1].replace(",", "").trim());
        return acc + price;
      }, 0);
      setRoomCharge(totalRoomCharge);
    }
  }, []);
  ...
  return (
    <div className="checkout-page">
      ...
      <section className="third">
        ...
        <div className="t4">
          <button className="t4-button" onClick={handleProceedToPayment}>Online Payment</button>
      ...
      <section className="third-two">
        ...
        <div className="payn">
          ...
          <div>
            <p>Rs. {(roomCharge + 254).toFixed(2)}</p>
          </div>
        </div>
      </section>
    </div>
